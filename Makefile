...
lint:
	CMD="lint charts/$(NAME)" $(MAKE) helm

dep-up:
	CMD="dep up charts/$(NAME)" $(MAKE) helm
...
ifdef CHART_VERSIONS
	CMD="package charts/$(NAME) --version $(word 1, $(CHART_VERSIONS)) -d packages" $(MAKE) helm
else
...
images:
	CHART=charts/$(NAME) $(MAKE) -s extracted-images annotated-images | sort -u
